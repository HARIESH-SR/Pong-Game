...
while True:
    ...
    if ball.ycor()>335:
        ball.sety(335)
        winsound.PlaySound("hit_ground.wav",winsound.SND_ASYNC | winsound.SND_ALIAS)
        ball.dy *= -1

    if ball.xcor()>435:
        ball.setx(435)
        winsound.PlaySound("hit_wall.wav",winsound.SND_ASYNC | winsound.SND_ALIAS)
        ball.dx *= -1
        score_a+=1
        pen.clear()
    ...
    if score_a == Winning_point:
        ...
        left_paddle.clear()
        ball.clear
        winsound.PlaySound("game_win.wav",winsound.SND_ASYNC | winsound.SND_ALIAS)
        pen.write("Player A Wins",align="center",font=("Comic Sans MS",24,"bold"))
        break

    elif score_b == Winning_point:
        pen.clear()
        pen1.clear()
        right_paddle.clear()
        left_paddle.clear()
